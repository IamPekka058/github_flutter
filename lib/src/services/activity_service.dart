...
class ActivityService extends Service {
  ...
  Stream<Event> listEventsPerformedByUser(String username, {int? pages}) {
    return PaginationHelper(
      github,
    ).objects('GET', '/users/$username/events', Event.fromJson, pages: pages);
  }
  ...
  Future<bool> markThreadRead(String threadId) {
    return github.request('PATCH', '/notifications/threads/$threadId').then((
      response,
    ) {
  ...
  Future<bool> isStarred(RepositorySlug slug) {
    return github.request('GET', '/user/starred/${slug.fullName}').then((
      response,
    ) {
